  ...
  const club = {
    ...
    demoClub() {
      return { id: 'demo-myclub', name: 'Club Indoor Madrid', invite_code: 'MAD7TL',
               owner_id: 'me', plan: 'pro', sport: 'tennis', city: 'Madrid', country: 'ES', crest: '🎾', color: '#E8703D' };
    },
    demoMembers() {
      return [
    ...
    identityFields(v) {
      ...
      const sp = v.sport || ((TL.extras && TL.extras.sportPref && TL.extras.sportPref()) || 'tennis');
      return `
        <input id="club-name" placeholder="${t('club_name_ph')}" value="${esc(v.name || '')}"/>
        <div class="seg2 club-sport-seg" id="club-sport">
          <button type="button" data-sp="tennis" class="${sp==='tennis'?'on':''}">🎾 ${t('sport_tennis')}</button>
          <button type="button" data-sp="padel" class="${sp==='padel'?'on':''}">🥎 ${t('sport_padel')}</button>
          <button type="button" data-sp="pickle" class="${sp==='pickle'?'on':''}">🏓 ${t('sport_pickle')}</button>
        </div>
    ...
    bindColors(scope) {
      const sportSeg = scope.querySelector('#club-sport');
      if (sportSeg) sportSeg.querySelectorAll('button').forEach(b => b.onclick = () => {
        sportSeg.querySelectorAll('button').forEach(x => x.classList.toggle('on', x === b));
      });
    ...
    readIdentity(scope) {
      ...
      const sp = scope.querySelector('#club-sport .on');
      return {
        sport: sp ? sp.dataset.sp : 'tennis',
    ...
    async createClub(meta) {
      ...
      const base = { owner_id: uid(), name: meta.name, invite_code: code,
                  city: meta.city || null, country: meta.country || null,
                  crest: meta.crest || null, color: meta.color || null };
      let res = await db().from('clubs').insert(Object.assign({}, base, { sport: meta.sport || 'tennis' })).select().maybeSingle();
      if (res.error && /sport/i.test(res.error.message || '')) {  // columna aún no migrada → reintenta sin deporte
        res = await db().from('clubs').insert(base).select().maybeSingle();
      }
      if (res.error) return { error: res.error.message };
      this.current = res.data; this.role = 'coach';
      return { ok: true };
    },
    ...
    async updateIdentity(meta) {
      ...
      const base = { name: meta.name, city: meta.city || null, country: meta.country || null,
        crest: meta.crest || null, color: meta.color || null };
      let { error } = await db().from('clubs').update(Object.assign({}, base, { sport: meta.sport || 'tennis' })).eq('id', this.current.id);
      if (error && /sport/i.test(error.message || '')) {  // columna aún no migrada → reintenta sin deporte
        ({ error } = await db().from('clubs').update(base).eq('id', this.current.id));
      }
    ...
    draw() {
      ...
      if (!this.current) {
        ...
        const _csp = this.current.sport || 'tennis';
        const sportChip = `<span class="club-sport-chip">${_csp==='padel'?'🥎':_csp==='pickle'?'🏓':'🎾'} ${esc(_csp==='padel'?t('sport_padel'):_csp==='pickle'?t('sport_pickle'):t('sport_tennis'))}</span>`;
          ...
          <div class="club-world-hero">
            ...
            <div class="club-world-id">
              ...
              <div class="club-world-meta">
                ...
                ${sportChip}
                ${(coach && !this.isPro()) ? `<button class="btn btn-primary btn-sm club-plan-cta" id="club-upgrade-hero">${en ? 'Get Club Plan' : 'Plan Club'}</button>` : ''}
    ...
    bind() {
      ...
      const upHero = h.querySelector('#club-upgrade-hero');
      if (upHero) upHero.onclick = () => this.upgradeClubPlan();
    ...
    async fillTactics() {
      ...
      box.innerHTML = summary + rows.map(r => {
        ...
        const sport = (r.data && r.data.sport === 'padel') ? '🥎' : (r.data && r.data.sport === 'pickle') ? '🏓' : '🎾';
    ...
    pickToShare() {
      ...
      const h = this.host();
      const opts = all.map(tc => `<option value="${tc.id}">${(tc.sport === 'padel' ? '🥎 ' : tc.sport === 'pickle' ? '🏓 ' : '🎾 ')}${esc(tc.name || t('untitled'))}</option>`).join('');
